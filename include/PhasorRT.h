...
extern "C"
{
	 ...
	 *
	 * @param script (in) A string containing the Phasor source to compile.
	 * @param moduleName The name of the module, used for error reporting.
	 * @param modulePath (in) An optional path to the parent folder for the script, used for resolving compile time imports.
	 * @param buffer (out) A pointer to a buffer where the compiled bytecode will be written. If null, the function will only calculate the required buffer size and return it via `outSize`.
	 * @param bufferSize (in) The size of the provided buffer. This is ignored if `buffer` is null.
	 * @param outSize (out) If `buffer` is null, this will be set to the required buffer size to hold the compiled bytecode. If `buffer` is not null, this will be set to the actual size of the compiled bytecode.
	 * @returns if buffer is null, returns true if compilation succeeded. if buffer is not null, returns true if compilation succeeded and buffer was valid and large enough to hold the compiled bytecode.
	 *
	 ...
	 *
	 * @param script (in) A string containing the Pulsar source to compile.
	 * @param moduleName The name of the module, used for error reporting.
	 * @param buffer (out) A pointer to a buffer where the compiled bytecode will be written. If null, the function will
	 * only calculate the required buffer size and return it via `outSize`.
	 * @param bufferSize (in) The size of the provided buffer. This is ignored if `buffer` is null.
	 * @param outSize (out) If `buffer` is null, this will be set to the required buffer size to hold the compiled
	 ...
	 * @param resetFunctions Resets functions if set to true.
	 * @param resetVariables Resets variables if set to true.
	 * 
