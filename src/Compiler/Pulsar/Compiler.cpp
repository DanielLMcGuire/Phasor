...
int Compiler::compileToBytecode()
{
	...
	{
		std::println(std::cerr, "Error: Cannot compile a bytecode file");
		return 1;
	}
	...
	if (!file.is_open())
	{
		std::println(std::cerr, "Could not open file: {}", m_args.inputFile);
		return 1;
	}
...
int Compiler::compileToIR()
{
	...
	{
		std::println(std::cerr, "Error: Cannot compile a Phasor IR file");
		return 1;
	}
...
void Compiler::showHelp(const std::string &programName)
{
	...
	std::println("Pulsar Compiler (Phasor v{})\n"
