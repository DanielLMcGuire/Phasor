...
-   `CCompiler/` - CLI wrapper for the native compilation path: generate a C++ header (via CppCodeGenerator), compile it to an object file, and link it against the phasor-runtime DLL. Supports header-only, object-only, and generate-only modes.
