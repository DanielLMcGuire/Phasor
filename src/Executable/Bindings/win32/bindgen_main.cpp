...
ParamInfo resolveParam(const std::string& type, std::string name, [[maybe_unused]] int index)
{
    ParamInfo p;
    p.varName = std::move(name);
    p.allowNull = false;

    if (type.starts_with("LP"))
    {
        std::string base = type.substr(2);
        if (StructRegistry.contains(base) != 0u)
        {
            p.isStruct = true;
            p.structName = base;
            p.isPointer = true;
    ...
    if (type.size() > 6 && type.starts_with("const "))
    {
...
Function parseFunction(const std::string &line, int lineNumber)
{
    ...
    if (StructRegistry.contains(f.returnType) != 0u) 
    {
...
void generateWrapper(const Function &f, std::ostream &out)
{
    ...
    for (size_t i = 0; i < f.params.size(); ++i)
    {
        ...
        if (p.isStruct)
        {
            ...
            if (p.allowNull)
            {
                ...
                if (p.expectedType == 1) 
                { 
                    typeCheckFunc = "phasor_is_bool";
                } else if (p.expectedType == 2) { 
                    typeCheckFunc = "phasor_is_int";
                } else if (p.expectedType == 3) { 
                    typeCheckFunc = "phasor_is_float";
                } else if (p.expectedType == 4) { 
                    typeCheckFunc = "phasor_is_string";
                } else if (p.expectedType == 5) { 
                    typeCheckFunc = "phasor_is_struct";
                }
    ...
    for (size_t i = 0; i < f.params.size(); ++i)
    {
        ...
        if (f.params[i].isStruct) 
        { 
            out << "&local_" << f.params[i].varName;
        } else { 
    ...
    if (f.isReturnStruct)
    {
        ...
        { 
            out << "\"" << sd.members[i].name << "\"" << (i == sd.members.size()-1 ? "" : ", ");
        }
        out << "};\n    PhasorValue r_vals[] = {";
