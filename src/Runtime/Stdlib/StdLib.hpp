...
class StdLib
{
  public:
	...
	/// @brief Validates that v is a string and returns it.
	static void requireString(const Value &v, const char *fnName, const char *what);

	/// @brief Validates that v is an integer and returns it.
	static void requireInt(const Value &v, const char *fnName, const char *what);

	/// @brief Validates that v is a bool and returns it.
	static void requireBool(const Value &v, const char *fnName, const char *what);

	/// @brief Validates that v is a number and returns it.
	static void requireNumber(const Value &v, const char *fnName, const char *what);

	/// @brief Validates that v is an array and returns it.
	static void requireArray(const Value &v, const char *fnName, const char *what);

	/// @brief Validates that v is a struct and returns it.
	static void requireStruct(const Value &v, const char *fnName, const char *what);

	/// @brief Validates that v is a non-zero integer pointer and returns it.
	static Phasor::i64 requireAddress(const Value &v, const char *fnName, const char *what = "address");
	/// @brief Validates that v is a positive integer length and returns it.
	static Phasor::i64 requireLength(const Value &v, const char *fnName, const char *what = "length");

	static char **argv; ///< Command line arguments
	static int    argc; ///< Number of command line arguments

	/// @brief Validates command line arguments for stdlib functions.
...
#pragma region stddata
	static Phasor::string base64_decoder(const Value::ArrayInstance &args, VM *vm);
	static Phasor::string base64_encoder(const Value::ArrayInstance &args, VM *vm);
	static Phasor::string md5(const Value::ArrayInstance &args, VM *vm);
	static Phasor::string sha1(const Value::ArrayInstance &args, VM *vm);
	static Phasor::string sha256(const Value::ArrayInstance &args, VM *vm);

	/// @brief Reads `length` bytes of native memory at `address` and base64-encodes them. (read-only)
	static Phasor::string base64_encoder_native(const Value::ArrayInstance &args, VM *vm);
	/// @brief Decodes a base64 string and writes the raw bytes into native memory. Returns bytes written. (write-only)
	static i64       base64_decoder_native(const Value::ArrayInstance &args, VM *vm);
	/// @brief Hashes `length` bytes of native memory at `address` with MD5. (read-only)
	static Phasor::string md5_native(const Value::ArrayInstance &args, VM *vm);
	/// @brief Hashes `length` bytes of native memory at `address` with SHA1. (read-only)
	static Phasor::string sha1_native(const Value::ArrayInstance &args, VM *vm);
	/// @brief Hashes `length` bytes of native memory at `address` with SHA256. (read-only)
	static Phasor::string sha256_native(const Value::ArrayInstance &args, VM *vm);
#pragma endregion
