...
Value windows::GetSecurityDescriptorHandle(const std::vector<Value> &args, VM *vm)
{
	...
	return vhandle::vhandleGuid(vhandle::createVHandle(sd, vhandle::HandleType::Generic, false));
}
...
Value windows::WrapSecurityDescriptorHandle(const std::vector<Value> &args, VM *vm)
{
	...
	return vhandle::vhandleGuid(
	    vhandle::createVHandle(reinterpret_cast<SECURITY_DESCRIPTOR *>(rawPtr), vhandle::HandleType::Generic, false));
}
...
// *SECURITY_ATTRIBUTES; SHAPED: VHANDLE<GENERIC> GetSecurityAttributesHandle([in] DWORD<int> nLength, [in]
// VHANDLE<GENERIC> lpSecurityDescriptor, [in] BOOL<bool> bInheritHandle);
...
#pragma region File
// VHANDLE<KERNEL> CreateFileA([in] LPCSTR<string> lpFileName, [in] DWORD<int> dwDesiredAccess, [in] DWORD<int>
// dwShareMode,
...
Value windows::GetProcAddress(const std::vector<Value> &args, VM *vm)
{
	...
	return vhandle::vhandleGuid(
	    vhandle::createVHandle(reinterpret_cast<void *>(procAddr), vhandle::HandleType::Procedure, true));
}
