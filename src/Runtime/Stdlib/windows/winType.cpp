...
namespace win {
    ...
    LPCSTR asLPCSTR(const Value& v) {
        if (!v.isString()) throw std::runtime_error("Expected string");
    ...
    intptr_t asINTPTR(const Value& v) {
        if (v.isFloat()) throw std::runtime_error("Expected integer, not float");
    ...
    template<typename T>
    T asHandle(const Value& v) {
        if (!v.isString()) throw std::runtime_error("Expected GUID string");
        vhandle::VHANDLE handle = vhandle::guidToVHandle(v.asString());
        return vhandle::getHandle<T>(handle);
    }

    template<typename T>
    T asOptionalHandle(const Value& v) {
        if (v.isNull()) return nullptr;
        return asHandle<T>(v);
    }
    ...
    void* asBuffer(const Value& v, size_t& size) {
        if (!v.isString()) throw std::runtime_error("Expected string buffer");
        std::string str = v.asString();
        size = str.length();
