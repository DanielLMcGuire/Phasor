...
static PhasorValue phasor_open(PhasorVM* vm, int argc, const PhasorValue* argv) {
    if (argc < 2 || !phasor_is_string(argv[0]) || !phasor_is_int(argv[1])) return phasor_make_int(-1);
...
static PhasorValue phasor_close(PhasorVM* vm, int argc, const PhasorValue* argv) {
    if (argc < 1 || !phasor_is_int(argv[0])) return phasor_make_int(-1);
...
static PhasorValue phasor_read(PhasorVM* vm, int argc, const PhasorValue* argv) {
    if (argc < 2 || !phasor_is_int(argv[0]) || !phasor_is_int(argv[1])) return phasor_make_string("");
    int fd = (int)phasor_to_int(argv[0]);
...
static PhasorValue phasor_write(PhasorVM* vm, int argc, const PhasorValue* argv) {
    if (argc < 2 || !phasor_is_int(argv[0]) || !phasor_is_string(argv[1])) return phasor_make_int(-1);
    int fd = (int)phasor_to_int(argv[0]);
...
static PhasorValue phasor_unlink(PhasorVM* vm, int argc, const PhasorValue* argv) {
    if (argc < 1 || !phasor_is_string(argv[0])) return phasor_make_int(-1);
...
static PhasorValue phasor_execve(PhasorVM* vm, int argc, const PhasorValue* argv) {
    ...
    for (size_t i = 0; i < argcnt; i++) args[i] = (char*)phasor_to_string(argarr[i]);
    args[argcnt] = NULL;
    for (size_t i = 0; i < envcnt; i++) envs[i] = (char*)phasor_to_string(envarr[i]);
    envs[envcnt] = NULL;
...
static PhasorValue phasor_waitpid(PhasorVM* vm, int argc, const PhasorValue* argv) {
    if (argc < 2 || !phasor_is_int(argv[0]) || !phasor_is_int(argv[1])) return phasor_make_int(-1);
